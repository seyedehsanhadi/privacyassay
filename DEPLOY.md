...
- Serve over HTTPS. The kit also runs from `file://` for a quick local look, but the two-origin cross-site test needs a real origin and is skipped there, so a `file://` run gives single-site numbers only. An earlier note here claimed LibreWolf reads lower on `file://` than over http; re-measured on both, it scores the same 43 either way, so that claim is withdrawn. It held only while a browser could be credited for a window that happened to land on the letterbox grid.
